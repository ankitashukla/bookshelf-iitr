 window.onload = function () {
                ...
                function curve(x, y, ax, ay, bx, by, zx, zy, color) {
                    var path = [["M", x, y], ["C", ax, ay, bx, by, zx, zy]],
                        path2 = [[0,0,0], ["L", ax, ay], ["M", bx, by], ["L", zx, zy]],
                        curve = r.path(path).attr({stroke: color || Raphael.getColor(), "stroke-width": 4, "stroke-linecap": "round"}),
                        controls = r.set(
                            r.path(path2).attr({stroke: "#ccc", "stroke-dasharray": ". "}),
                            r.circle(0,0,0).attr(discattr),
                            r.circle(0,0,0).attr(discattr),
                            r.circle(0,0,0).attr(discattr),
                            r.circle(0,0,0).attr(discattr)
                        );
                ...
                curve(170, 20, 110, 100, 170, 200, 130, 200, "hsb(0, .75, .75)");
                // curve(170, 100, 210, 100, 230, 200, 270, 200, "hsb(.8, .75, .75)");
                // curve(170, 100, 110, 100, 170, 200, 130, 200, "hsb(.3, .75, .75)");
